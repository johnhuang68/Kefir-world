[English](roadmap.md) | **繁體中文**

# Kefir World 研究路線圖

## 已完成的 profile 研究 — 2026-09-06 至 2026-09-08

- [x] 為 Caspian Sea Yogurt、Viili、Långfil、Filmjölk、Matsoni、Dahi 與 Bulgarian Yogurt 建立詳細 profile。
- [x] 為上述產品補上物種／菌株證據、EPS、製程、儲存與不確定性說明。
- [x] 更新七筆目錄資料，加入 profile 連結、參考文獻程式碼與明確的發酵條件範圍。
- [x] 擴充共用參考文獻登入表，並保留原本的牛奶克菲爾文獻。
- [x] 以限定主張範圍的原始或權威來源，加強 Piimä、Tettmelk、Laban、Labneh、Rayeb、Tarag、Katyk 與 Varenets。
- [x] 新增 Khoormog、Pendidam 與 Kindirmou，作為各自有來源支援的傳統發酵乳。

目前 46 筆產品資料都有依主張範圍整理的文獻 profile。這些是已完成的第一輪綜整，尚不是完整的實驗菌株登入紀錄。優先缺口包括：Långfil 與 Tettmelk 的發酵時間、Filmjölk 與芬蘭 Piimä 的菌株來源、長期回種研究，以及成對的儲存／酸化測量。未來的結構化菌株表必須保留 Matsoni 後續的基因組重新鑑定與文獻間不一致的分離株拼法。

## 第一階段 — 核心目錄

- [x] 建立全球發酵乳初步目錄。
- [x] 加入實用發酵溫度範圍。
- [x] 加入典型發酵時間。
- [x] 分類菌種生態與傳代方式。
- [x] 每一列至少附上一個可靠來源。
- [x] 在 `fermentation_scope` 與 profile 中分開記錄家戶／傳統製程與工業製程範圍。

## 第二階段 — 微生物學

為每一種菌種／產品加入：

- 乳酸菌的屬與物種
- 酵母
- 醋酸菌
- 與傳統製程相關的黴菌
- 產生 EPS 的微生物
- 優勢與偶見分類群
- 培養依賴型證據與定序證據
- 有可靠文獻支援的菌株層級識別碼

優先研究：

- 牛奶克菲爾菌粒微生物組與地理差異
- kefiran 產生菌
- Viili／Långfil 的拉絲性與 EPS
- 裡海優格與 *Lactococcus lactis* subsp. *cremoris* 菌種系譜
- Matsoni／Matzoon 的區域微生物組差異
- Kumis／Airag 的乳酸菌－酵母生態
- Dadih、Mabisi、Nunu、Amasi 等自然發酵或回種系統

## 第三階段 — 發酵行為

在有證據時加入量化欄位：

- 初始與終點 pH
- 可滴定酸度
- 乳糖降低量
- 乳酸
- 乙醇
- CO2
- 黏度
- EPS／kefiran 產量
- 菌粒生長速率
- 乳清析出
- 香氣化合物
- 多種溫度下的發酵動力學

## 第四階段 — 製程變因

追蹤下列因素的影響：

- 全脂乳與低脂乳
- 牛、山羊、綿羊、水牛、駱駝與馬乳
- 巴氏殺菌乳與 UHT 乳
- 蛋白質濃度
- 脂肪濃度
- 接種比例
- 發酵容器
- 氧氣暴露
- 攪拌
- 回種代數
- 冷藏
- 冷凍乾燥／脫水

## 第五階段 — 地理資料

為下列地區建立地理紀錄：

- 高加索
- 北歐國家
- 巴爾幹
- 中東
- 中亞
- 南亞
- 東南亞
- 東非
- 西非
- 南部非洲

後續成果：建立互動式世界地圖，顯示各地發酵乳傳統、溫度範圍、使用乳種與菌種生態。

## 第六階段 — 證據資料庫

建議的參考文獻表欄位：

`source_id, product, title, authors, year, journal_or_institution, doi_or_url, study_type, geography, milk_type, starter, fermentation_temperature, fermentation_time, microbial_method, key_findings, evidence_quality`

精確菌株、溫度與微生物量化主張應優先引用原始論文；綜述與 FAO／Codex 文獻適合支援類別定義與歷史背景。

## 第七階段 — 視覺化

建立：

- 發酵溫度範圍圖
- 發酵時間與溫度比較圖
- 菌種生態比較
- 乳酸菌／酵母／醋酸菌矩陣
- EPS 與質地比較
- 世界發酵乳地圖
- 菌種傳代決策樹
- 互動式產品比較網站

## 長期願景

Kefir World 應發展成多語言、可追溯證據的發酵乳圖鑑，而不只是優格名稱清單。每項主張最終都應能連回來源；每種傳統產品也應保留區域差異，不被簡化成一套通用配方。
